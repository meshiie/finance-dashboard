...
// ── Adhoc alerts — new charges since yesterday ────────────────────────────────
function detectNewAdhocCharges(adhocBills, allTxns) {
  ...
  for (const bill of adhocBills) {
    ...
    // Calculate running total this cycle
    ...
    for (const charge of newCharges) {
      alerts.push({
        ...
        cycle_total: parseFloat(cycleTotal.toFixed(2)),
        budget: bill.amount,
        over: cycleTotal > bill.amount,
      });
    }
  }
  return alerts;
...
async function gatherData() {
  ...
  const budgetComparison = budgetObjs
    ...
    .map(b=>({
      category:b.category,
      spent:discretionaryCats[b.category]||0,
      budget:b.fnBudget,
  ...
  const uncategorised = allTxns
    .filter(t=>t.direction==="debit"&&t.date>=cycle.thisCycleStart&&
      (t.cat===""||t.cat==="nan"||!t.cat)&&!isMortgage(t.desc)&&t.amount>1)
    .sort((a,b)=>b.amount-a.amount).slice(0,8);

  // Adhoc bills — pass cycleStart for running total calculation
...
function buildDailyPrompt(d) {
  ...
  const projected = d.projectedDiscretionary>0
    ?`$${d.projectedDiscretionary.toFixed(0)} projected at current pace`:"first day of cycle";
...
function buildFortnightlyPrompt(d) {
  ...
  const catChanges = Object.keys({...d.discretionaryCats,...d.prevDiscretionaryCats})
    .map(cat=>{
      const curr=d.discretionaryCats[cat]||0, prev=d.prevDiscretionaryCats[cat]||0, diff=curr-prev;
...
function buildAdhocAlertMessage(charges, dateStr, timeStr) {
  const lines = charges.map(c => {
    const budgetNote = c.over
      ? `⚠️ Over $${c.budget} budget (total: $${c.cycle_total.toFixed(2)})`
      : `Running total: $${c.cycle_total.toFixed(2)} of $${c.budget} budget`;
...
async function main() {
  ...
  console.log(`🚗 New adhoc charges: ${data.newAdhocCharges.length}`);
  ...
  // Then send daily or fortnightly summary
  let summary, header;
  if (data.isPayday) {
